...
async function main() {
  for (let i = 0; i < 284; i++) {
    ...
    const totpop = await fetch (`https://population.un.org/dataportalapi/api/v1/data/indicators/49/locations/${loc.id}/start/2022/end/2022`);
    const dta = await totpop.json();
    if (!dta.data || !dta.data.length) { continue }
    const totalpop = dta.data[0].value;
  
    const popchg = await fetch (`https://population.un.org/dataportalapi/api/v1/data/indicators/50/locations/${loc.id}/start/2022/end/2022`);
